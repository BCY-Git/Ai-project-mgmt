...
# AI Project Management System - Deploy to Server Script with Password Authentication
# This script uploads files to the server and triggers deployment using SSH password
...
# Default password (can be overridden with -p flag)
DEFAULT_PASSWORD="qwsdcv123!@#"
...
usage() {
    ...
    echo ""
    echo "Options:"
    echo "  -p, --password PASSWORD  SSH password for authentication (default: qwsdcv123!@#)"
    echo "  -u, --user USER          SSH user (default: root)"
    echo "  -s, --server IP          Server IP address (default: 106.54.44.45)"
    ...
    echo "  $0"
    echo "  $0 -p mypassword"
    echo "  $0 -u admin -p mypassword -s 192.168.1.100 -P 2222"
}
...
install_sshpass() {
    if ! command -v sshpass &> /dev/null; then
        print_warning "sshpass is not installed. Installing it now..."

        # Detect OS
        if [[ "$OSTYPE" == "darwin"* ]]; then
            # macOS
            print_status "Installing sshpass on macOS..."
            if command -v brew &> /dev/null; then
                ...
                echo ""
                echo "Alternatively, you can manually install sshpass:"
                echo "  1. Download from: https://sourceforge.net/projects/sshpass/files/"
                echo "  2. Extract and run: ./configure && make && sudo make install"
                exit 1
            fi
        else
            # Linux
            if command -v apt-get &> /dev/null; then
                ...
                print_error "Unsupported Linux distribution. Please install sshpass manually."
                exit 1
            fi
        fi
        ...
        print_status "sshpass is already installed."
    fi
}
...
rsync_with_pass() {
    ...
    rsync -avz --progress -e "sshpass -e ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p ${SERVER_PORT}" "$@"
    unset SSHPASS
}
...
print_step "CHECKING SSH AUTHENTICATION TOOLS"
install_sshpass

# Test SSH connection
print_status "Testing SSH connection to server with password..."
if ! ssh_with_pass -p ${SERVER_PORT} -o ConnectTimeout=10 ${SERVER_USER}@${SERVER_IP} "echo 'Connection successful'" 2>/dev/null; then
    print_error "Cannot connect to server via SSH"
    echo ""
    echo "Please ensure:"
    echo "1. Server IP (${SERVER_IP}) is correct"
    echo "2. SSH port (${SERVER_PORT}) is open"
    echo "3. Password authentication is enabled on the server"
    echo "4. The provided password is correct"
    echo ""
...
echo "Using Password Authentication: YES"
...
ssh_with_pass -p ${SERVER_PORT} ${SERVER_USER}@${SERVER_IP} "
...
" | grep -q "FIRST_DEPLOYMENT=true"; then
    ...
    if [ -f "${PROJECT_PATH}/server-setup.sh" ]; then
        scp_with_pass -P ${SERVER_PORT} "${PROJECT_PATH}/server-setup.sh" ${SERVER_USER}@${SERVER_IP}:/root/
        ssh_with_pass -p ${SERVER_PORT} ${SERVER_USER}@${SERVER_IP} "
            chmod +x /root/server-setup.sh
            echo '${PASSWORD}' | sudo -S ./server-setup.sh
        "
        ...
        print_error "server-setup.sh not found. Please run it manually on the server."
    fi
fi
...
ssh_with_pass -p ${SERVER_PORT} -t ${SERVER_USER}@${SERVER_IP} "
    cd ${SERVER_PATH}
...
CONTAINER_STATUS=$(ssh_with_pass -p ${SERVER_PORT} ${SERVER_USER}@${SERVER_IP} "
    cd ${SERVER_PATH}
...
echo "[$(date)] Deployment ${DEPLOYMENT_TIMESTAMP} to ${SERVER_IP} completed (password auth)" >> "$LOG_FILE"
